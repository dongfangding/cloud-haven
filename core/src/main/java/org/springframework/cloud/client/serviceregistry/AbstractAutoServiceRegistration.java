...
import com.snowball.cloud.haven.core.rest.RegisterCenterController;
...
public abstract class AbstractAutoServiceRegistration<R extends Registration>
    ...
    public void start() {
        ...
        int times = 100;
		String url = "http://localhost:" + port.get() + contextPath + "/register-center/ping";
        while (times > 0) {
            try {
                // 经过测试，DispatcherServlet是在WebServerInitializedEvent事件后初始化的，但是如果在容器启动后，但是DispatcherServlet又没初始化的情况下，调用接口请求,
                // 会使用tomcat线程池来触发这个初始化，所以按照这个流程的话，似乎注册中心并不存在这样的间隔时间问题，导致接口无法使用
                final String result = HttpUtil.get(url);
                if (Objects.equals(RegisterCenterController.SUCCESS, result) || Objects.equals("\"SUCCESS\"", result)) {
                    log.info("注册中心-通过接口检查~");
                    break;
                } else {
                    log.error("注册中心-接口尚未初始化成功, url = {}, result = {}", url, result);
                }
                times--;
                Thread.sleep(1000);
            } catch (Exception e) {
				times--;
                log.error("注册中心-调用ping接口失败, url = {}", url, e);
            }
        }
        if (!isEnabled()) {
