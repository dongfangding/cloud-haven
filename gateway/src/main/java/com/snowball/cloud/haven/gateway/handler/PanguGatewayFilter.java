...
public class PanguGatewayFilter implements GlobalFilter {
    ...
    private Map<String, String> resolveClientHeaders(ServerHttpRequest request, HttpHeaders headers,
        ...
        // 处理服务端内部的请求头
        clientHeaderMap.put(RequestHeaderEnum.CLIENT_IP_FROM_GATEWAY.getName(), getClientIp(request));
        clientHeaderMap.put(RequestHeaderEnum.USER_ID_FROM_GATEWAY.getName(),
                Objects.nonNull(userClaim) ? userClaim.getUserId() : headers.getFirst(RequestHeaderEnum.IMEI.getName())
        );
        clientHeaderMap.put(RequestHeaderEnum.IS_GATEWAY_DISPATCH.getName(),
        ...
        clientHeaderMap.put(RequestHeaderEnum.IS_CONSOLE_WHITELIST_IMEI.getName(),
    ...
    private Map<String, String> resolveServerHeaders(ServerWebExchange exchange, UserClaim userClaim) {
        ...
        serverHeaderMap.put(
                RequestHeaderEnum.TRACE_ID_FROM_GATEWAY.getName(), generateTraceId(
                        Objects.nonNull(userClaim) ? userClaim.getUserId() :
                                headers.getFirst(RequestHeaderEnum.IMEI.getName())));
        serverHeaderMap.put(RequestHeaderEnum.IS_CONSOLE_WHITELIST_IMEI.getName(),
                RequestHeaderEnum.IS_CONSOLE_WHITELIST_IMEI.getDefaultValue()
        );
